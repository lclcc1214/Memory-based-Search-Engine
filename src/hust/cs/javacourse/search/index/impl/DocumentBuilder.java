...
public class DocumentBuilder extends AbstractDocumentBuilder {
    ...
    public AbstractDocument build(int docId, String docPath, File file) {
        try{
            // 使用装饰者模式，实现过滤功能，过滤长度过长的词、含有非英文字符的词以及停用词
            BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(new FileInputStream(file)));
            AbstractTermTupleStream termTupleStream = new StopWordTermTupleFilter(new PatternTermTupleFilter(new LengthTermTupleFilter(new TermTupleScanner(bufferedReader))));
            // 调用上面定义的build(docId, docPath, termTupleStream)方法构造Document对象
